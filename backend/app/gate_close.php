...
if ($check_email == 1) { 
    ...
    // $res = $conn->prepare("SELECT `time` FROM `gate_auth`  WHERE `gate_open_by` = '$username'  ORDER BY `id` DESC LIMIT 1 ");
    // $res->execute();
    ...
    // $res = $conn->prepare("SELECT `gate_open_time` FROM `gate_auth` WHERE `gate_open_by` = '$username'  ORDER BY `id` DESC LIMIT 1 ");
    // $res->execute();
    ...
    $res = $conn->prepare("SELECT `id` FROM `gate_auth` WHERE `gate_open_by` = '$username'  ORDER BY `id` DESC LIMIT 1 ");
    $res->execute();
    $res->bind_result($id);
    while ($res->fetch()) {
        $id = $id;
    ...
    if ($status == "open") {
        ...
        $data .= ", `gate_auth_close_position` = '$position' ";
